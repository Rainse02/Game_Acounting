...
void main() {
  ...
  group('entries', () {
    ...
    test('getOrCreateGame updates category when existing game category changes', () async {
      final publisher = await db.getOrCreatePublisher('米哈游');
      final game1 = await db.getOrCreateGame(publisher.id, '原神', Categories.service);
      expect(game1.category, Categories.service);

      final game2 = await db.getOrCreateGame(publisher.id, '原神', Categories.library);
      expect(game2.id, game1.id);
      expect(game2.category, Categories.library);

      final allGames = await db.getAllGames();
      expect(allGames.single.category, Categories.library);
    });
  });
